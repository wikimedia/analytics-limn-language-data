...
cursor.execute("SELECT userAgent, wiki, event_sourceLanguage, event_sourceTitle, event_targetLanguage, event_targetTitle, event_token, event_trace, event_version FROM ContentTranslationError_11767097 WHERE event_context = 'save-failure' and timestamp like '" + timestamp + "%' GROUP BY event_session ORDER BY timestamp")
